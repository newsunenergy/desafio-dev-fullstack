...
const ModalLeadDetails = ({ id, closeModal }: ModalLeadDetailsProps) => {
  const { consume, fields, openConsume, setOpenConsume, leadsDetails, handleConsume } =
    ...
    <>
      <Dialog open={openConsume}>
        <DialogContent className="bg-box border border-textInput">
          <div className="flex justify-between items-center">
            ...
            <X className="h-4 w-4 cursor-pointer" onClick={() => setOpenConsume(false)} />
          </div>
      ...
      <DialogContent className=" max-w-[630px] bg-box border border-textInput">
        <div className="flex justify-between items-center">
          ...
          <X className="h-4 w-4 cursor-pointer" onClick={() => closeModal(false)} />
        </div>
              ...
              {leadsDetails?.unidades?.map((invoice) => (
                ...
                >
                  ...
                  <TableCell
                    ...
                    onClick={() => handleConsume(invoice.historicoDeConsumoEmKWH)}
                  >
                    Ver Consumos
                  </TableCell>
                </TableRow>
