...
export class AddLeadController implements Controller {
  constructor (private readonly addLeadUsecase: AddLead, private readonly checkLeadExists: CheckExistentLead) {}
  async handle (request: AddLeadController.Params): Promise<HttpResponse> {
    try {
      ...
      await this.checkLeadExists.leadExists({ email: request.email })
